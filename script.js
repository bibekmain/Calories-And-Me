...
function getImage(form){//this takes the form used to get user input as a parameter
  const formData = new FormData(form);
  let searchTerm = formData.get('name');
  var requestURL = "https://pixabay.com/api/?key=35844814-7d1b1acb06ab5ddd767b0ed30&q=" + searchTerm + "&image_type=photo&min_width=200&max_width=200&safesearch=true&per_page=3&category=food"
  
  fetch(requestURL)
    .then(resp => resp.json())
    .then(resp => {//do something with json recieved from pixabay
      imageURL = resp.hits[1].webformatURL; //selects the first image returned through API
      imgEl.setAttribute("src", imageURL);
      console.log(); //sets the src attribute of image in html
